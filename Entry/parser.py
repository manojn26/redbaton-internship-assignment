...
class ParserClass:
    ...
    def get_fields(self):
        ...
        for i in range(1,4):
            url = "https://news.ycombinator.com/news?p={0}".format(str(i))
            parse_data = str(requests.get(url=url).content)
            ...
            # Extract Title
            title = [i.text for i in soup.find_all("span",{"class":"titleline"})]


            # Extract Href's URL Value
            url = [i.find_next("a").get("href") for i in soup.find_all("span",{"class":"titleline"})]

            # Extract Upvotes
            upvotes = [int(i.text.split()[0])for i in soup.find_all("span",{"class": "score"})]
          
            # Author
            author = [i.text for i in soup.find_all("a",{"class": "hnuser"})]

            # Posted Time
            posted_on = [i.text for i in soup.find_all("span",{"class": "age"})]
            ...
            zipped = list(zip(url,title,hurl,author,posted_on,upvotes,comments))
