...
# Model Imports
from .models import NewsFeed, Profile, Bookmarks, UserDeleted, UserRead

# Forms and Parser Imports
...
def login_user(request):
    ...
    if request.method == "GET":
        ...
        if user is not None and user.is_active:
            login(request,user)
            return render(request,"entry/index.html",{"user":user})
...
@login_required
def profile(request,userval):
    context = {}

    if request.method == "GET" and request.user.is_active:
        user_ = User.objects.get(username = userval)
        prof_ = Profile.objects.get(user = userval)

        context['data'] = prof_

    return render(request,"entry/profile.html",context)
...
def marked_read(request):
    ...
    user = User.objects.get(username = request.user)
    prof = Profile.objects.get(user = user)
    marks = UserRead.objects.filter(userid = prof).values_list("hid_id")
    obj = NewsFeed.objects.filter(id__in = marks).order_by("posted_on")

    read_history['data'] = obj

    return render(request,"entry/history.html",read_history)


# 9. Mark a URL as Read
@login_required
def marked_read_details(request,hid):
    user = User.objects.get(username = request.user)
    prof = Profile.objects.get(user = user)
    nfd = NewsFeed.objects.get(hid = hid)

    if request.user.is_active and request.method == "POST" and hid is not None:
        try:
...
def bookmark(request):
    ...
    user = User.objects.get(username = request.user)
    prof = Profile.objects.get(user = user)
    marks = Bookmarks.objects.filter(user=prof).values_list("hack_id_id")

    obj = NewsFeed.objects.filter(id__in = marks).order_by("posted_on")

    bookmark["data"] = obj

    return render(request,"entry/bookmarks.html",bookmark)
...
# 12. Delete Item Logical Function
@login_required
def deleteitem(request,hid):
    if request.user.is_active and request.method == "POST":
        ...
        try:
            userdel = UserDeleted.objects.get(hid = hid,userid = prof_user)
            return HttpResponse(status = 204)
        except ObjectDoesNotExist:
            userdel = UserDeleted.objects(userid = prof_user,hid=hid_,deleted = True)
